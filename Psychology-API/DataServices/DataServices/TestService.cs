...
namespace Psychology_API.DataServices.DataServices
{
    public class TestService : TestRepository, ITestService
    {
        ...
        {
            return await base.CreateAndGetPatientTestResultRepositoryAsnyc(doctorId, patientId, testId, testResultInPoints, questionsAnswers);
        }

        public async Task<Test> GetTestAsync(int testId)
        {
            return await base.GetTestRepositoryAsync(testId);
        }

        public async Task<PatientTestResult> GetTestHistiryOfPatientAsync(int patientTestResultId)
        {
            return await base.GetTestHistiryOfPatientRepositoryAsync(patientTestResultId);
        }
        ...
        {
            return await base.GetTestsRepositoryAsync();
        }

        public async Task<IEnumerable<PatientTestResult>> GetTestsHistiryOfPatientAsync(int patientId)
        {
            return await base.GetTestsHistiryOfPatientRepositoryAsync(patientId);
        }
    }
}